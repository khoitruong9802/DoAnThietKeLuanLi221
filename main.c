...
//    }
//}
unsigned char isIncTimeButtonPressed()
{
    return (key_code[0] == 1) ? 1:0;    //Press '1' to increase digit value
}

unsigned char isConfirmButtonPressed() 
{
    return (key_code[1] == 1) ? 1:0;   //Press '2' to confirm change on set value mode
}

unsigned char isCautionButtonPressed() 
{
    return (key_code[2] == 1) ? 1:0;   //Press '3' to enter caution mode (Blinking yellow LEDs)
}

unsigned char isDecTimeButtonPressed()
{
    return (key_code[4] == 1) ? 1:0;    //Press '4' to decrease digit value
}

unsigned char isManualButtonPressed()
{
    return (key_code[5] == 1) ? 1:0;    //Press '5' to enter manual mode
}

unsigned char isEscButtonPressed()
{
    return (key_code[6] == 1) ? 1:0;    //Press '6' to quit/pass from some modes
}
...
//    }
    switch (status)
    {
        ...
        case RG:
            ...
            if (isIncTimeButtonPressed() == 1) { 
                clearOldInput(); 
                status = SET_YELLOW; 
                UartSendString("-----SET TIME MODE-----\r\n");
            }
            if (isManualButtonPressed() == 1) status = MANUAL_RED;
            if (isCautionButtonPressed() == 1) status = CAUTION;
            break;
        case RY:
            //LED = 0b00100010;
            OpenOutput(0);
        ...
        case SET_YELLOW:
            ...
            if (flag_10s) {
                ...
                if (isDecTimeButtonPressed() == 1) {
                    valueInput[1]--;
                    if (valueInput[1] < 1) valueInput[1] = 9;
                    set_state = 1;
                    cnt_10s = 0;
                }
                if (isEscButtonPressed() == 1) {
                    clearOldInput();
                    status = SET_GREEN;
                    UartSendString("You escaped from set yellow mode without saving.\r\n");
                }
                else {
                    if (isConfirmButtonPressed() == 1) {
                        TIME_Y = valueInput[1];
        ...
        case SET_GREEN:
            ...
            if (flag_10s) {
                ...
                if (isDecTimeButtonPressed() == 1) {
                    ...
                    if (set_state == 1) {
                        valueInput[0]--;
                        if (valueInput[0] < 1) valueInput[0] = 9;
                    }
                    else if (set_state == 2) {
                        valueInput[1]--;
                        if (valueInput[1] < 0) valueInput[1] = 9;
                    }
                    cnt_10s = 0;
                }
                if (isEscButtonPressed() == 1) {
        ...
        case MANUAL_RED:
            ...
            if (isManualButtonPressed() == 1) {
                status = MANUAL_YELLOW_1;
                print_flag = 0;
            }
            if (isCautionButtonPressed() == 1) status = CAUTION;
            if (isEscButtonPressed() == 1) status = INIT_SYSTEM;
            break;
        case MANUAL_GREEN:
            LcdClearS();
            LcdPrintStringS(0, 0, "GREEN");
